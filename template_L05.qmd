...
Create a recipe with more advanced feature engineering that predicts `wlf` using all other variables and the following steps
  ...
  - add two-way interactions between all predictor variables
  - add a **P**rincipal **C**omponent **A**nalysis (PCA) step (could try kernel PCA as well) in order to reduce the dimensionality after adding all two-way interactions. Tune the number of components as well by flagging it with `tune()`. 
